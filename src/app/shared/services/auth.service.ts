...
export class AuthService {
  ...
  login(email: string, googleId: string): Observable<any> {
    return this.http
      .get<any>(`${environment.student_api}?email=${email}&googleId=${googleId}`)
      .pipe(// can thieu du lieu tra  ve observable
        map((item) => { // bien doi du lieu tra ve thanh mot du lieu khac
          // console.log(item);
          // return item;
